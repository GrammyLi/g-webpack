...
const parsedEntry = (entry) => {
  ...
  /*
  {
    '../helper/utils.js': '/Users/lichao/Desktop/g-webpack/demo/helper/utils.js',
    '../helper/dom.js': '/Users/lichao/Desktop/g-webpack/demo/helper/dom.js'
  }
  **/
  //   console.log("ds", ds);
...
// Plugin 的作用是扩展打包工具的功能。
const bundle = (entry, output, plugins = []) => {
  plugins.forEach((plugin) => {
    if (plugin.beforeParse) plugin.beforeParse(entry);
  });
  // 获取依赖图
...
//   console.log("================================ graph", graph);
  ...
  // 生成 bundle.js 的字符串代码
  let bundleCode = bundleTemplate(module);

  plugins.forEach((plugin) => {
    if (plugin.afterBundle) plugin.afterBundle(bundleCode);
  });
  // log('bundle', bundle)
  saveBundle(bundleCode, output);
};
